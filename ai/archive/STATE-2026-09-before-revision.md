...
- **2026-08-28 — İki denetim raporu uygulandı; prim'in ölçüm taşımadığı bulundu**
  (**ADR #014**, ders **L-020**). Bağımsızlık nöbetçisi · rejim dejenerelik
  kapısı · `notify` takvim kapısı · CI workflow'u. Test 855 → **872**.
  Detay: `ai/archive/STATE-2026-08.md`.
- **2026-09-02 — cron gecikmesi gün etiketini kaydırıyormuş** (**ADR #015**,
  ders **L-021**). Gecikme TR gece yarısını aşınca iş kendini ertesi günün işi
  sanıyordu; ölçülen hasar: `rapor_2026-08-31.md` **kayıp**, `rapor_2026-08-27.md`
  hiç oluşmadı, 08-31 Pazartesi **mutabakatı koşmadı**, `predictions`ta
  **asof=2026-08-26 yok** — üçü de Actions'ta YEŞİL'di. Koşunun günü artık
  `util.slot_gunu()`. Test 872 → **880**. Aynı kök BIST'te de vardı (BIST L-026).

## 🔨 Devam Edenler
- _(yok)_

## ⚠️ İZLENECEK — `archive.yml` cron ritmi (2026-08-28)
`archive.yml` koşum sayısı 24/gün → **2/gün**e düştü; koşumlar başarısız
DEĞİL, hiç tetiklenmiyor (GitHub throttling). `archive_observed_freq_minutes:
90` bu dalgalanmaya göre yeniden kalibre EDİLMEDİ — payda bayat, metrik
gerilemelerini gizleyebilir (açık iş). Detay: `ai/archive/STATE-2026-08.md`
ve `İZLEME.md` → "Actions cron ritmi".

## 🧱 Bekleyenler (iş değil, zaman)
- 🟡 **👤 Mert — eksik `asof=2026-08-26` doldurulsun mu?** Geriye dönük
  yazılmadı: geç kararı `kaynak="canli"` saymak karneyi bozar (ADR #014).
  (a) boş kalsın · (b) `kaynak="gec"` ile yazılıp karne filtresinden düşsün.
