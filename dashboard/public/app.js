...
function renderContent() {
  ...
  let content='';
  if (view==='overview') content=`<div class="grid"><div>${chartPanel(s)}${decisionPanel(s)}</div><div class="side-column">${hero(s)}${metricsPanel(s)}${newsPanel(s)}</div></div>`;
  if (view==='decisions') content=decisionPanel(s,true)+costs(s);
  if (view==='portfolio') content=positions(s)+trades(s)+chartPanel(s)+costs(s);
  if (view==='learning') content=learningPage(s);
